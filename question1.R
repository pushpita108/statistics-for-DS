...
#sum(countDf$freq)
plot(x=countDf$FROMNODE, y= countDf$freq, main="Number of tokens sold by addresses", sub="Sellers",xlab="FROM NODE", ylab="TOKEN AMOUNT")

#graph2
countFromDf <- count(mydata, "TONODE")
plot(x=countDf$TONODE, y= countDf$freq, main="Number of tokens bought by addresses", sub="Buyers",xlab="TO NODE", ylab="TOKEN AMOUNT")
# 
