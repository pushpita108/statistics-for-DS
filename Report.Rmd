...
```{r}
#getting the unique users in the current token

#buyers <- unique(mydata$TONODE) 
#sellers <- unique(mydata$FROMNODE)
#buyers <- append(buyers, sellers)
#buyers <- data.frame(unique(buyers))
#colnames(buyers) <- c("USERS")
buyers1 <- (mydata$TONODE) 

sellers1 <- (mydata$FROMNODE)

buyers12 <- append(buyers1, sellers1)
buyers12 <- data.frame(buyers12)
colnames(buyers12) <- c("USERS")
usersFreq <- count(buyers12, "USERS")
meanFreq <- mean(usersFreq$freq)
activeUsers <- usersFreq[(usersFreq$freq>meanFreq),]
#reading all the data from other tokens into a single dataframe
col_names <- c("FROMNODE","TONODE","DATE","TOKENAMOUNT")
...
dataframe_list <- list()
t <- data.frame(read.csv( files[1], header = FALSE, sep = " ", dec = ".", col.names = col_names))
dataframe_list[[1]] <- t
for(i in 2:length(files)){
  t <- data.frame(read.csv( files[i], header = FALSE, sep = " ", dec = ".", col.names = col_names))
  tusers <- unique(t$FROMNODE)
  tbuyusers <- unique(t$TONODE)
  tusers <- append(tusers, tbuyusers)
  tusers <- data.frame(unique(tusers))
  colnames(tusers)<-c("TOKENUSERS")
  dataframe_list[[i]] <- tusers
}
...
```{r}
df <- ldply(dataframe_list, data.frame)
length(df$FROMNODE)
df
```

```{r}
coName <- c("FROMNODE", "COUNT")
x <- table(df$TOKENUSERS[df$TOKENUSERS %in% activeUsers$USERS]) # counting the occurances of the users in the FROMNODE column of other tokens
sellerData <- data.frame(x)
colnames(sellerData)<-coName
sellerData
#sellerData
#maxVal<-  function(data) sapply(data, max, na.rm = TRUE)
#val<-max(sellerData$FROMNODE,na.rm = TRUE)
#val
val<-apply(sellerData,2,max)
val
#mydata
```
...
```{r}
#filteredCountData<-subset(sellerData, FROMNODE %in% activeUsers$"USERS")
countOfFreq <- count(sellerData, "COUNT")
colnames(countOfFreq) <- c("Users_Count", "Freq_Count")
descdist(countOfFreq$Freq_Count, boot= 500)

```

```{r}
distributionFit_Count_pois <- fitdist(countOfFreq$Freq_Count, "pois", method ="mle")
distributionFit_Count_wb <- fitdist(countOfFreq$Freq_Count, "weibull", method ="mle")
distributionFit_Count_ln <- fitdist(countOfFreq$Freq_Count, "lnorm", method ="mle")
distributionFit_Count_gm <- fitdist(countOfFreq$Freq_Count, "gamma" ,method="mme")
