...
col_names <- c("FROMNODE","TONODE","DATE","TOKENAMOUNT")
fpath<-"B:/study/Fall 2018/Stats for DS/Project1/Datasets/Ethereum token graphs/"
...
x <- table(df$TOKENUSERS[df$TOKENUSERS %in% buyers$USERS]) # counting the occurances of the users in the TONODE column of other tokens
sellerData <- data.frame(x)
colnames(sellerData)<-coName
head(sellerData)
maxVal<-  function(sellerData) sapply(sellerData, max, na.rm = TRUE)
maxVal
```

```{r}
buycoName <- c("TONODE", "COUNT")
x <- table(df$TOKENUSERS[df$TOKENUSERS %in% buyers$USERS]) # counting the occurances of the users in the FROMNODE column of other tokens
buyerData <- data.frame(x)
colnames(buyerData)<-buycoName
head(buyerData)
